...
- The toggle button is rendered in `Navbar.tsx` in both the desktop pill nav and the mobile top-bar action row.
...
- **Dock nav**: `app/components/ui/Dock.tsx` — a generic `Dock`/`DockItem` pair, icon-only nav items without a hover label popover. On desktop (`sm:` and up), the icons magnify (34px → 50px) based on cursor proximity along the x-axis, spring-smoothed (`useSpring`); `disableMagnify` is passed `shouldReduceMotion` so the size stays fixed at 34px under reduced motion. `Navbar.tsx` feeds it `links: DockItem[]` (`{ href, label, icon }`, one `lucide-react` icon per section).
- **Mobile**: The top bar shows only the logo plus Download Resume, Hire Me, and theme actions. The six section links live in a persistent, centered liquid-glass bottom dock with magnification disabled for touch. Because `.liquid-glass-panel` establishes `position: relative`, the dock sets `position: fixed` inline in `Navbar.tsx` to keep it pinned to the viewport. `Footer` adds bottom padding below `sm` so its content remains accessible above that fixed dock; all other grids stack via Tailwind responsive classes (`sm:`, `lg:`).
...
Applied to: `ThemeToggle` button; `Navbar`'s desktop nav links/"Hire Me" pill and mobile top-bar/bottom-dock controls; `TechBadge` pills; `Contact`'s per-item tiles; `SectionHeading`'s eyebrow pill (every section); `GlassCard` (every section panel — About, Experience, Projects, Skills, Education, Contact); `TiltIcon` (every icon instance — Experience, Skills, Education, Contact); About's three stat tiles (now wrapped in a small bordered/tinted surface, tinted per-stat with violet/cyan/amber glows); Education's "Beyond Work" interest pills (tinted violet, matching that subsection's icon accent).
...
- `.liquid-glass-panel` — rim + sheen only, **no lift/scale transform**, for big containers that themselves hold other `.liquid-glass*` elements (the outer section `GlassCard`s, the top navbar pill, and mobile bottom dock). CSS `:hover` bubbles up from any descendant, so giving a large panel the same lift as its own children would make the whole panel bob every time something inside it is hovered — this variant keeps the ambient glass glint without that jitter.
...
- **Fitts's Law**: `ThemeToggle` and the mobile resume action are 40×40px (`h-10 w-10`), closer to the ~44px recommended minimum touch target. Desktop nav link vertical padding increased slightly (`py-1.5` → `py-2`) for a taller hit area without changing visual density.
...
`profile.resume` (`app/data/resume.ts`) points at `/Bhargavlal_KrishnaReddy_Resume.pdf`, served from `public/Bhargavlal_KrishnaReddy_Resume.pdf`. A "Download Resume" button (`download` attribute, `lucide-react`'s `Download` icon) appears in `Hero.tsx`'s CTA row and in `Navbar.tsx`'s desktop and mobile top bars. **`public/Bhargavlal_KrishnaReddy_Resume.pdf` currently ships as a placeholder stub — replace it with the real resume PDF (same filename) before shipping**; no component changes are needed when swapping the file.
